...
import { authTokenHandler } from "../middlewares/checkAuthToken";
...
router.get("/getclassbyid/:classid", authTokenHandler, async (req, res) => {
  ...
  try {
    let classroom;
    if (
      classroom.owner == req.userid ||
      classroom.students.includes(req.userid)
    ) {
      classroom = await Classroom.findById(classid).populate("posts");
      //.populate means-- in model we have added post ref, it will populate that
    }
...
router.post("/addpost", authTokenHandler, async (req, res) => {
  ...
  try {
    ...
    if (classroom.owner.toString() !== req.userid) {
      return res
        .status(403)
        .json({ message: "Not allowed to share post in this class" });
    }
